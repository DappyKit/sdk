...
describe('Optimism Mainnet Verification Gateway', () => {
  ...
  it('should respond with error during Google verification', async () => {
    ...
    const owner = createRandomWallet(mnemonic)
    const sdk = await createSdk(optimismMainnetConfig, mnemonic)

    expect(await sdk.gateway.verification.verifyGoogle('one', await owner.signMessage({ message: 'two' }))).toEqual({
      status: 'error',
